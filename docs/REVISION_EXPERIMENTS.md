...
the horizon. The original clustered-delay setting was subsequently found to be
physically unattainable by every comparator and is retained only as an archived
calibration, not as reviewer-facing evidence. Its frozen primary replacement is
`aw_recoverable_clustered_late_returns`, generated by
`build_followup_disturbance_cases.py`. Multi-step price escalation reaches the model through the
disturbance workbook rather than through a notice, because the physical-event
schema carries fleet and charger updates but no prices.

The follow-up multi-day study chains three 48-timestep daily horizons and copies
each bus's exact realized terminal energy into the next day's initial state. A
temporary charger-cooling derating begins on the afternoon of day 1, persists
through day 2, and clears on the afternoon of day 3. It is a rolling-horizon
experiment, not a single 144-step perfect-foresight optimization. A matched
no-derating control performs a scheduled replan at each daily handover. Every
daily workbook and carryover record has a SHA-256 run signature over its inputs,
prompts, code, model, and solver controls; unsigned or mismatched prior outputs
are recomputed. Physical feasibility is evaluated independently of the
altruistic revenue-retention design constraint.

```powershell
python scripts/build_extended_disturbance_cases.py
python scripts/run_advance_warning_matrix.py --output-root results/revision/extended_disturbances_v3 --ablation-protocol inputs/revision/advance_warning_ablation_protocol_extended_v1.json --notices-file inputs/revision/advance_warning_notices_v2.json --physical-events-file inputs/revision/advance_warning_physical_events_v2.json --case aw_extended_energy_shift --include-agent --agent-repetitions 5 --allow-external-llm --require-clean-git --solver-time-limit 300 --max-approximate-api-cost-usd 2.00

python scripts/build_followup_disturbance_cases.py
python scripts/run_advance_warning_matrix.py --output-root results/revision/recoverable_cluster_v1 --ablation-protocol inputs/revision/advance_warning_recoverable_cluster_protocol_v1.json --notices-file inputs/revision/followup_notices_v1.json --physical-events-file inputs/revision/followup_physical_events_v1.json --case aw_recoverable_clustered_late_returns --include-agent --agent-repetitions 5 --allow-external-llm --solver-time-limit 300 --max-approximate-api-cost-usd 1.00

python scripts/build_multiday_charger_derating.py
python scripts/run_multiday_charger_derating.py --agent-repetitions 5 --allow-external-llm --solver-time-limit 300 --max-approximate-api-cost-usd 3.00
python scripts/analyze_multiday_solver_audit.py

# Targeted time-limit sensitivity; no API calls
python scripts/run_multiday_charger_derating.py --output-root results/revision/multiday_nominal_selfish_900s_v1 --configuration oracle_event_trigger --mode selfish --solver-time-limit 900 --solver-mip-gap 0.02
python scripts/analyze_multiday_solver_audit.py --output-root results/revision/multiday_nominal_selfish_900s_v1
...
The completed multi-day matrix contains 18 three-day episodes and 54 daily
runs. All are operationally feasible, with zero reserve shortfall, zero exact
internal carryover error, and at most 0.0000492 kWh first-settlement discrepancy
from four-decimal observation output. All ten Trigger-Agent repetitions skip
the unconfirmed warning at timestep 22 and optimize on its confirmation at
timestep 24, before the charger cap begins at timestep 31. The pre-confirmation
context has no energy-disturbance flag because interval depletion is compared
with the active real-time plan. Trigger methods tie economically after the
advance warning is incorporated. The no-derating comparison shows accumulating
day-specific SOC and trading effects from the derating; it is a physical-effect
control, not an Agent-performance baseline. All 120 optimizer attempts use
Gurobi, 118 meet the 2% target, and the two selfish nominal day-3 attempts retain
feasible time-limited incumbents at approximately 5.5% gap. Economic deltas that
use that nominal cell are therefore reported as approximate.
The targeted 900-second sensitivity reaches 5.50% and 5.42% gaps but retains
the identical schedule and economics to numerical precision. Tripling the
limit therefore confirms incumbent stability while providing little progress
toward the 2% proof target.
